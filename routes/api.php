...
//     ->name('api.payment-intent.create');

// Route::post('/order-by-payment-intent', [PaymentIntentController::class, 'getOrderByPaymentIntent'])
//     ->name('api.order.by-payment-intent');

// // Stripe Webhook (exclude from CSRF - see step 6)
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    // admin-style product/category APIs
    Route::post('/products',        [ProductController::class, 'store']);
    Route::put('/products/{id}',    [ProductController::class, 'update']);
    Route::delete('/products',      [ProductController::class, 'destroyByQuery']);
    Route::delete('/products/{id}', [ProductController::class, 'destroy']);
